...
export const formataSamsungMadrid = (produtoLoja: IProdutoLoja): IProdutoLoja | null => {
    ...
    novoNome = novoNome.replace(/CELULAR|CEL|DUAL SIM|ESCURO|CORE|SEM LACRE|LCD|S LACRE|A\.LED|S CX S ACS| S LA| S L|\(C.F.\)|\(NO\)|\(C.F\)|DP(?!.*DP)|MP(?!.*MP)|SS(?!.*SS)|K(?!.*K)|DG(?!.*DG)|NFC(?!.*NFC)|CRE(?!.*CRE)|\s\+(?!.*\s\+)|\.(?!.*\.)|\(.*(?!\().*$/gi, '').replace(/CAM13\+2\+2 5|CAM13\+2\+2MP 5|\s+/g, ' ').trim();
