...
export const formataXiaomiMobileZone = (produtoLoja: IProdutoLoja): IProdutoLoja | null => {
    ...
    novoNome = novoNome.replace(/CELULAR|CEL|DUAL SIM|ADAPTADOR|C ROTA|LTE|ATLA|\+ADATAP|\(VITRINA\)|\*\*(?!.*\*\*)|\*(?!.*\*)|\S L(?!.*\S L)|C FEA(?!.*C FEA)|\-(?!.*\-)|US(?!.*US)|DP(?!.*DP)|K(?!.*K)|DG(?!.*DG)|NFC(?!.*NFC)|L P-(?!.*L P-)|L P(?!.*L P)|G(?!.*G)|\.(?!.*\.)|\S CAPA(?!.*\S CAPA)|\s\+(?!.*\s\+)/gi, '').replace(/\s+/g, ' ').trim();
