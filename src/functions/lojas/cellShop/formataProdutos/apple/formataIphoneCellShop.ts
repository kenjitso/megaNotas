...
export const formataIphoneCellShop = (produtoLoja: IProdutoLoja): IProdutoLoja | null => {
    const marca =
    ...
    if (cor) novoNome = novoNome.replace(/VERDE|AZUL|CINZA|PRETO|LITE GREEN|BRANCO|ONYX BLACK|S\.BLACK|BLACK|GLACIER BLUE|MINT GREEN|ICE BLUE|TWILIGHT BLUE|STAR BLUE|COSMIC PURPLE|ONYX GRAY|BRONZE|LIGHT BLUE|LAVANDER PURPLE|GRAPHITE GRAY|OCEAN BLUE|LAVENDER PURPLE|PRATA|PEPPY PURPLE|AMARELO|FOREST GREEN|PINK|ROSA|BLUE|GOLD|VERMELHO|GRAPHITE|YELLOW|MIDNIGHT|DOURADO|DEEP PURPLE|STARLIGHT|PURPLE|SILVER|WHITE|GREEN|SPACE|ROSE/gi, '');
    ...
    novoNome = novoNome.replace(/CELULAR|DUAL SIM|CEL|OLED|LCD|SCS|D\.|\(NANOSIM|CPO(?!.*CPO)| A(?!.* A)/gi, '').replace(/SCS|\s+|\(\)/g, ' ').trimStart();
